...
> **DANGER**: **Sacrificial Processes:** It is critical to inject stolen tickets into a "sacrificial process" rather than overwriting the ticket of an existing logon session. Overwriting a ticket for a critical process (like `SYSTEM$`) or a service can cause an outage, as the service may not be able to re-authenticate until it is restarted. Using a sacrificial process creates a new, isolated logon session for the ticket. While this is safer, creating a new logon session (`LOGON_TYPE = 9`) is an Indicator of Compromise (IOC) that can be detected.
